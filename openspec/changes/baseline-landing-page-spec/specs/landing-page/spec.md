...
The page SHALL provide three scroll-related behaviors implemented in `script.js`: a header background/color swap after 40px of scroll, a one-time reveal-on-scroll animation for elements marked `.reveal` (via `IntersectionObserver`, threshold 0.12), and smooth in-page scrolling for same-page anchor links.
